...
public:
		...
		~MovieList(){
			Movie *temp = head;
			if (temp == NULL)
			cout << "The list is empty - nothing to destruct\n";
			while(temp != NULL){
				temp = head->get_next_movie();
				delete head;
				head = temp;
			}
		...
		void add_Movie(int id, char *title, int year) {
			...
			while (temp -> get_next_movie()  != NULL && (strcmp(temp -> get_next_movie() -> get_title() , node->get_title() )) < 0 ){ 
				temp = temp -> get_next_movie(); 
			}
